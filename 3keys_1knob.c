...
void handle_key(uint8_t current, struct key * key, uint8_t * neo) {
  if(current != key->last) {                // state changed?
    ...
    if(current) {                           // key was pressed?
      ...
      if(key->type == KEYBOARD) {
        KBD_code_release(key->mod, key->code); // release keyboard/keypad key
      } else {
        CON_release(key->code);             // release consumer key
      }
    }
  }
...
void main(void) {
  ...
  for (i = 0; i < 6; i++) {
    ...
    keys[i].type = (char)eeprom_read_byte(i * KEY_EEPROM_FIELDS + 1);
  ...
  while(1) {
    ...
    if(currentKnobKey) {
      if (currentKnobKey->type == KEYBOARD) {
        KBD_code_type(currentKnobKey->mod, currentKnobKey->code);
      } else {
        CON_type(currentKnobKey->code);
      }
    }
